...
## ソースコードの説明
l.75 文字の設定
l.76~78 for文の入れ子を使って表を初期化
l.81 表の縦を見ていく。（つまり商品を１つ１つ見ていく）
l.82~83 表の横を見ていく。商品が入らないときは１つ上のデータを見ていく。
l.86 さっきの続き（商品が入る時）を考える。
l.87　商品を入れ替えない場合と入れ替えた場合を比較しもし入れ替えた時のカロリーが大きいなら
l.88 入れ替えたときの値を入れる。
l.90　入れ替えないときが大きいなら(else)
l.91　入れ替えないとき（１つ上の値）を入れる。
l.95 表の一番右下を見て最大カロリーの値を表示する。(printf文)
